    ...
    <nav className="sticky top-0 z-50 flex h-[3.5625rem] w-full items-center justify-between border-b border-[#c9c1b5]/80 bg-[#f4f1eb]/88 px-5 backdrop-blur-xl sm:px-8 lg:px-[max(2rem,calc((100vw-1180px)/2))]">
      ...
      <div className="flex items-center gap-3 whitespace-nowrap text-muted-foreground sm:gap-7">
        <Link href="/macro-matcher">
          <p className="text-[0.68rem] transition-colors hover:text-foreground sm:text-xs">
            Macro <span className="hidden sm:inline">Matcher</span>
          </p>
        </Link>
        ...
        <span className="mx-1 h-4 w-px bg-[#c9c1b5]" aria-hidden="true" />
        ...
        {auth?.user ? (
          <Link href="/dashboard" className="flex items-center gap-1.5">
            <UserCircle
              size={20}
              weight="duotone"
              className="text-[#4A7865]"
            />
            <p className="hidden text-[0.68rem] transition-colors hover:text-foreground sm:block sm:text-xs">
              {auth.user.username}
            </p>
          </Link>
        ) : (
          <Link href="/login" className="flex items-center gap-1.5">
            <SignIn size={16} className="sm:hidden" />
            <p className="hidden text-[0.68rem] transition-colors hover:text-foreground sm:block sm:text-xs">
              Sign in
            </p>
          </Link>
